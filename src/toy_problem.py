...
def load_data():
    x_train  = np.load("../data/sets/toy/x_train.npy")
    y_train  = np.load("../data/sets/toy/y_train.npy")
    x_test   = np.load("../data/sets/toy/x_test.npy")
    y_test   = np.load("../data/sets/toy/y_test.npy")
    mu_test  = np.load("../data/sets/toy/mu_test.npy")
    var_test = np.load("../data/sets/toy/var_test.npy")
    return (x_train, y_train, x_test, y_test, mu_test, var_test)

def generate_data():
    x_train  = np.reshape(np.random.uniform(0.0, 2.0 * np.pi, 100000), (-1,1))
    y_train  = np.random.normal(np.sin(x_train), 0.1 + np.sin(0.5 * x_train)).ravel()
    x_test   = np.reshape(np.random.uniform(0.0, 2.0 * np.pi, 10000), (-1, 1))
    y_test   = np.random.normal(np.sin(x_test), 0.1 + np.sin(0.5 * x_test)).ravel()
    mu_test  = np.sin(x_test)
    var_test = 0.1 + np.sin(0.5 * x_test)

    np.save("../data/sets/toy/x_train.npy", x_train)
    np.save("../data/sets/toy/y_train.npy", y_train)
    np.save("../data/sets/toy/x_test.npy", x_test)
    np.save("../data/sets/toy/y_test.npy", y_test)
    np.save("../data/sets/toy/mu_test.npy", mu_test)
    np.save("../data/sets/toy/var_test.npy", var_test)
